...
exports.get_dashboard_datas = ()=>{
    ...
    query = "SELECT * FROM TBL_AGENT_INFO T1 INNER JOIN TBL_GROUP_SET_LIST T2 ON T1.AGENT_CD = T2.AGENT_CD INNER JOIN TBL_GROUP_INFO T3 ON T2.GROUP_SET_CD = T3.GROUP_SET_CD WHERE T3.ACTIVE_STATE = 'A'AND T1.DEL_FLAG = 0 AND T3.DEL_FLAG = 0 ORDER BY CREATE_TIME DESC";
...
exports.get_dashboard_top10 = ()=>{
    ...
    query = "SELECT TOP 10 COUNT(*) cnt, T2.INSPECT_ITEM_CD AS item_cd FROM TBL_INSPECT_STATS T1 INNER JOIN TBL_INSPECT_SURVEY T2 ON T1.INSPECT_CD = T2.INSPECT_CD INNER JOIN TBL_GROUP_SET_LIST T3 ON T1.GROUP_SET_CD = T3.GROUP_SET_CD INNER JOIN TBL_GROUP_INFO T4 ON T3.GROUP_SET_CD = T4.GROUP_SET_CD INNER JOIN TBL_AGENT_INFO T5 ON T3.AGENT_CD = T5.AGENT_CD WHERE T2.INSPECT_RESULT = 0 AND T4.DEL_FLAG = 0 AND T5.DEL_FLAG = 0 AND T4.ACTIVE_STATE = 'A' GROUP BY INSPECT_ITEM_CD ORDER BY cnt DESC ";
