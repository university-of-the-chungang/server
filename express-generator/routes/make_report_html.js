...
exports.buildHtml = (row, row1) => {
    return new Promise((resolve, reject) => {
        ...
        fs.readFile(__dirname + "/sample.html", "utf-8", (err, html) => {
            ...
            html = html.replace(/\$\{GROUP_NO\}/gi, row["GROUP_SET_CD"]);
...
<div>
<abbr title="The target system or system component did not satisfy at least one condition of the rule.">success</abbr>
</div>
</td>
</tr>`;
...
<div>
<abbr title="The target system or system component did not satisfy at least one condition of the rule.">unknown</abbr>
</div>
</td>
</tr>`;
...
<a data-toggle="modal" href="#myModal"> ${title_arr[i]}</a>
            ...
            //html = html.replace(/\$\{checkpoint_arr\}/gi,JSON.stringify(checkpoint_arr));
            //html = html.replace(/\$\{fix_arr\}/gi,JSON.stringify(fix_arr));
            html = html.replace(/\$\{severity_arr\}/gi,JSON.stringify(severity_arr));
